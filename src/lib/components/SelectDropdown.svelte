<script lang="ts">
  ...
  const resolvedButtonClass = $derived(
    buttonClass ??
      `inline-flex ${SIZE_CLASS[size].minW} cursor-pointer items-center justify-between gap-2 rounded-md border border-slate-700 bg-slate-950 px-3 text-slate-100 outline-none transition hover:border-cyan-500 focus:border-cyan-500 ${SIZE_CLASS[size].pad} text-${size}`,
  )

  const resolvedControlClass = $derived(
    controlClass ??
      `${SIZE_CLASS[size].minW} field-sizing-content cursor-pointer rounded-md border border-slate-700 bg-slate-950 pl-3 pr-8 text-slate-100 outline-none transition hover:border-cyan-500 focus:border-cyan-500 ${SIZE_CLASS[size].pad} text-${size}`,
  )

  const optionRowClass = $derived(
    optionClass ??
      `flex w-full cursor-pointer items-center justify-between rounded-md px-3 text-left outline-none transition ${SIZE_CLASS[size].pad} text-${size}`,
  )
