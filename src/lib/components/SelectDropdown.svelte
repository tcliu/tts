<script lang="ts">
  ...
  // `text-${size}` below is interpolated, which Tailwind's scanner cannot see.
  // The four candidates are force-generated by the `@source inline(...)` rule in
  // src/styles.css; keep that rule in sync when adding a size (asserted by
