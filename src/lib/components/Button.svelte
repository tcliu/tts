<script lang="ts">
  ...
  // `relative` anchors the `before:-inset-1.5` (text) / `before:-inset-2` (sm icon-only) hit-area expansion: do not remove it,
...
{/snippet}
  ...
  {#if tooltip}
    <Tooltip align={tooltipAlign} trigger={buttonEl}>{tooltip}</Tooltip>
  {/if}
