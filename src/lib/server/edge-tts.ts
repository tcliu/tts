...
// pair is a single character, so the transform is length-preserving and keeps
// word-boundary offsets aligned with the original text. Pairs verified against
// the zh-HK lexicon (variants it already pronounces are left out on purpose).
// Kokuji and ambiguous forms (雫 辻 込 峠 働 払) have no clean Chinese
// equivalent and are deliberately not mapped; only a ja voice can read them.
